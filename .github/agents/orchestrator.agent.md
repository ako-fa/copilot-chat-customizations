...
| `@review`    | コードレビュー       | 実装後の品質チェック（3 回サイクル管理）              |
...
以下のエージェントは、上記のドメインオーケストレーターから共通で利用される。直接呼び出しも可能。

| エージェント        | 役割         | 説明                                                 |
| ------------------- | ------------ | ---------------------------------------------------- |
| `@research`         | 情報収集     | コードベース走査・ドキュメント調査・外部リソース検索 |
| `@deep-analyze`     | 深掘り分析   | 技術的な内部構造・依存関係・副作用の分析             |
| `@critical-analyze` | 批判的分析   | 前提の穴・盲点・リスク・矛盾の指摘                  |
| `@synthesize`       | 統合・構造化 | 複数の調査・分析結果を統合し構造化された結論を生成   |
| `@validate`         | 基準検証     | チェックリストベースの合否判定                        |
| `@report`           | レポート作成 | 標準化された形式で報告書を作成                       |
