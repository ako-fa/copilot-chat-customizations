...
- レイアウト、スペーシング、レスポンシブ対応は Tailwind のユーティリティクラスで定義する
- `@keyframes`、複雑なグラデーション、疑似要素、複雑なセレクタは SCSS で定義する
- Tailwind と SCSS の担当領域を明確に分離し、同一責務を重複実装しない
- GSAP 連携時は CSS カスタムプロパティを介して値を受け渡し、状態管理クラスで補助する
- SCSS ファイルは `app/assets/style/scss/` 配下に配置する
...
1. スタイル対象が Tailwind と SCSS のどちらの担当かを先に判定する
2. Tailwind 担当の場合は、ユーティリティクラスでレイアウト、スペーシング、レスポンシブを記述する
3. SCSS 担当の場合は、`app/assets/style/scss/` または対象コンポーネントの scoped style に記述する
4. GSAP 連携が必要な場合は、CSS カスタムプロパティを介して値を接続し、必要に応じて状態クラスで補助する
5. Tailwind と SCSS の担当が混在していないかを確認する

## チェックリスト

- レイアウト、スペーシング、レスポンシブが Tailwind で実装されている
- 複雑なアニメーション、疑似要素、複雑なセレクタが SCSS で実装されている
- Tailwind と SCSS で同一責務を重複実装していない
- GSAP 制御箇所で CSS transition の競合を回避できている
- GSAP 連携時の値の受け渡しに CSS カスタムプロパティを利用している
- SCSS の配置場所が `app/assets/style/scss/` ルールに従っている
