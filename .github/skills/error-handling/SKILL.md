...
- エラーは原因別に分類し、利用者向け文言と運用者向け情報を分離する
- 例外を握りつぶさず、fail fast で早期に検知して処理を打ち切る
- `any` を禁止し、`unknown` と型ガードで安全に絞り込む
- 一時的障害はリトライし、恒久的障害は即時に失敗として扱う
- ログには再現に必要な最小情報のみを残し、機密値を出力しない
...
1. 失敗し得る処理を洗い出し、ネットワーク、バリデーション、認証・認可、システムの 4 区分で分類する
2. 区分ごとにカスタムエラー型と共通レスポンス構造を定義し、UI 表示と運用ログの責務を分離する
3. 非同期処理にリトライとフォールバックの方針を適用し、最大試行回数と失敗時の挙動を固定する
4. `unknown` と型ガード、判別可能ユニオンで分岐の網羅性を担保し、`any` を排除する
5. 例外経路を含むテストを実行し、ログに機密値が含まれていないことを確認してから反映する

## チェックリスト

- エラー分類が処理全体で一貫しているか
- 利用者向けメッセージと運用者向け詳細ログが分離されているか
- `any` を使わず、`unknown` と型ガードで安全に処理しているか
- fail fast で無効入力や異常状態を早期検出しているか
- リトライ上限、フォールバック条件、最終失敗時の挙動が明文化されているか
- ログやレスポンスにトークン、パスワード、個人情報が出力されていないか
