...
- GSAP プラグイン登録は `app/plugins/gsap.client.ts` に集約し、アプリ全体で一度だけ実行する
- import は `gsap` と `gsap/all` のみを許可し、個別パスからの import は禁止する
- Composable では `useNuxtApp()` や `window` 経由を使わず、直接 import した `gsap` を利用する
- アニメーションのライフサイクルは必ず管理し、`onUnmounted` で `kill()` を実行して破棄する
- パフォーマンスのため `x` `y` `scale` `rotation` など `transform` 系プロパティと `opacity` を優先する
- `markers` は開発時の検証用途に限定し、本番環境では無効化する
...
1. `gsap.client.ts` で必要なプラグインが登録済みか確認する
2. `gsap` または `gsap/all` からのみ import する
3. `useGsap` composable でアニメーションを宣言的に記述する
4. `onUnmounted` での `kill` 処理を実装する
5. 開発時に `markers` で動作確認し、確認後に削除する

## チェックリスト

- `app/plugins/gsap.client.ts` にプラグイン登録が集約されている
- import 元が `gsap` または `gsap/all` のみになっている
- `useNuxtApp()` や `window` 経由で GSAP を参照していない
- `onUnmounted` で `Timeline` / `Tween` / `ScrollTrigger` を確実に破棄している
- アニメーション対象プロパティが `transform` 系と `opacity` 中心になっている
- 開発確認後に `markers` を無効化または削除している
