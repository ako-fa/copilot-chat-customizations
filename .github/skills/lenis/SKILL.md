...
- パッケージ名は `lenis` を使用し、`@studio-freight/lenis` は使用しない
- Lenis の初期化は `app/plugins/lenis.ts` と Composable 経由の利用パターンで一元管理する
- 二重初期化を禁止し、`useNuxtApp()` で既存インスタンスを参照する
- GSAP ScrollTrigger との同期は公式推奨パターンに従う
- 廃止・変更済みの旧オプションは使用せず、本ファイルの対応表に従って最新オプションを使用する
...
1. `lenis` パッケージが導入済みか確認する
2. `lenis/dist/lenis.css` を import する
3. `useLenis` composable で Lenis を初期化する
4. GSAP ScrollTrigger との同期を設定する
5. 廃止オプションを使用していないか確認する

## チェックリスト

- `@studio-freight/lenis` ではなく `lenis` を使用している
- `lenis/dist/lenis.css` を import している
- Lenis のインスタンスを二重初期化していない
- `useNuxtApp()` 経由で既存インスタンスを利用している
- GSAP ScrollTrigger との同期を公式推奨パターンで構成している
- 廃止・変更済みの旧オプションを使用していない
